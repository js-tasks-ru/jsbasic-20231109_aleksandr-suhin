...
export default class StepSlider {
  #steps;
  #value;
  #shiftX;//смещение курсора внутри бобышки
  ...
  #onPointerMove = (event) => {
    ...
    let left = event.clientX - this.#shiftX - slider.getBoundingClientRect().left;
    let leftRelative = left / this.elem.offsetWidth;
    // курсор вышел из слайдера => оставить бегунок в его границах.
    if (leftRelative<0) leftRelative = 0;
    if (leftRelative>1) leftRelative = 1;
    let step = Math.round(leftRelative * (this.#steps - 1)); // 0..4      
    this.#SetValue(step);
  }
  ...
  #onPointerUp = () => {
    ...
    //document.removeEventListener('pointerup', onPointerUp); //сделаем это автоматически {once : true}
  }
